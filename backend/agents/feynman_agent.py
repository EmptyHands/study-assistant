...
{{
  ...
  "recurring_weak_points": ["本轮暴露的、与历史记录一致的薄弱点（如果有）"],
...
class FeynmanAgent(BaseAgent):
    ...
    async def evaluate_answer(self, context: dict) -> dict:
        ...
        if history:
            ...
            parts = []
            for h in recent:
                prev_q = h.get("question", "")
                prev_a = h.get("answer", "")[:200] if h.get("answer") else "(未回答)"
                prev_eval = h.get("evaluation", {})
                level = prev_eval.get("understanding_level", "") if prev_eval else ""
                level_tag = f" [理解: {level}]" if level else ""
                parts.append(f"- 问: {prev_q}\n  答: {prev_a}{level_tag}")
            if parts:
                history_section = "之前的对话历史（含评估结果）:\n" + "\n".join(parts)

        prompt = EVALUATE_ANSWER_PROMPT.replace("{title}", context.get("title", "")).replace("{question}", context.get("question", "")).replace("{answer}", context.get("answer", "")).replace("{history_section}", history_section)
